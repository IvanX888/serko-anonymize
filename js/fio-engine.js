...
(function (root, factory) {
  ...
  /* Нормализация лемм фамилий: «бондаря»→«бондарь», «морозову»→«морозов» —
   * морфологический анализатор основы на -ь не знает и фиксирует форму,
   * из-за чего падежные формы не сливаются с именительной. */
  function normSurnameLemma(l) {
    if (!l || surnameSet == null) return null;
    if (surnameSet.has(l)) return null;
    var tries = [[/ия$/, 'ь'], [/я$/, 'ь'], [/а$/, ''], [/ого$/, 'ый'], [/ему$/, 'ый'], [/ым$/, 'ый'], [/им$/, ''], [/е$/, ''], [/у$/, ''], [/ю$/, '']];
    for (var i = 0; i < tries.length; i++) {
      var cand = l.replace(tries[i][0], tries[i][1]);
      if (cand !== l && surnameSet.has(cand)) return cand;
    }
    // фолбэк без словаря: -ь-основы и -ова/-ина. Слияние всё равно страхуют
    // инициалы (имя+отчество обязаны совпасть), ложных склеек почти нет
    if (/я$/.test(l) && l.length >= 5) return l.slice(0, -1) + 'ь';
    if (/а$/.test(l) && l.length >= 6) return l.slice(0, -1);
    return null;
  }
  ...
  function init() {
    ...
    surnameSet = new Set(('' + (Dict.surnames || '')).split(/[,|]/).map(function (w) { return w.toLowerCase(); }));
  ...
  function wordRole(word, genderHint) {
    ...
    for (var i = 0; i < analyses.length; i++) {
      ...
      // словарный анализ бьёт эвристический: фамилия «Тарасевич» из словаря
      // не должна проигрывать паттерну-отчеству (-вич)
      var pa = (a.source === 'dict' ? 100 : 0) + (priority[a.type] || 0) * 10 + ((genderHint && a.gender === genderHint) ? 5 : 0);
      var pb = (best.source === 'dict' ? 100 : 0) + (priority[best.type] || 0) * 10 + ((genderHint && best.gender === genderHint) ? 5 : 0);
      if (pa > pb) best = a;
    }
    return best;
  }
  ...
  function detectFio(text, options) {
    ...
    for (var i = 0; i < tokens.length; i++) {
      ...
      // одно слово после маркера — только словарная фамилия («Свидетель Сорока» ✓,
      // неизвестное слово «Адвокатского» ✗ — фантом)
      if (seq.length < 2 && !wordRole(seq[0].word)) continue;
  ...
  function scoreCandidate(text, cand, customWords, userStop) {
    ...
    var hasLower = false, hasDigit = false, hasLatin = false, hasCyr = false, hasStop = false, hasOrg = false, hasGeo = false;
    var dictHits = 0, patrHits = 0;
    for (var i = 0; i < parts.length; i++) {
      var w = parts[i];
      if (/^[А-ЯЁІЁа-яёіў]\.?$/.test(w)) continue;
      // «О.П.» — инициалы: cleanWord даст «оп», что коллидирует с оргформой «ОП»
      if (/^[А-ЯЁІЁа-яёіў]\.[А-ЯЁІЁа-яёіў]\.$/.test(w)) continue;
      if (/^[а-яёіў]/.test(w)) hasLower = true;
      if (/[0-9]/.test(w)) hasDigit = true;
      if (/[a-zA-Z]/.test(w)) hasLatin = true;
      if (/[А-ЯЁІЎа-яёіў]/.test(w)) hasCyr = true;
    ...
    if (hasDigit) { score -= 50; reasons.push('цифры'); }
    // латиница сама по себе не штрафуется: «(Ivanov I.I.)» — легитимная
    // транслитерация ФИО; штрафуем только смесь алфавитов в одном кандидате
    if (hasLatin && hasCyr) { score -= 30; reasons.push('смесь кириллицы/латиницы'); }
  ...
  function extractPersons(matches, text) {
    ...
    for (var i = 0; i < matches.length; i++) {
      ...
      for (var p = 0; p < parts.length; p++) {
        ...
        if (!role) {
          ...
          if (!person.surname && isCapitalizedWord(w)) {
            ...
            person.surnameSurface = w;
            person.surnameAfterInitials = person.initials.length > 0;
          }
          continue;
        }
        ...
        if (role.type === 'surname' && !person.surname) {
          ...
          person.surnameSurface = w; // v9.3: поверхностная форма (жен./муж. омонимия)
          person.surnameAfterInitials = person.initials.length > 0; // «И.И. Фамилия»
      ...
      if (person.surname) {
        var snNorm = normSurnameLemma(person.surname);
        if (snNorm) person.surname = snNorm;
      }
      ...
      // «Петровна» одна — не персона; но «Тарасевич И.В.» — фамилия на -вич
      // С инициалами — персона, без — фантом-отчество
      if (person.surname && !person.name && !person.patronymic && person.initials.length === 0 && /(вна|вич|ична)$/.test(person.surname)) continue;
      var key = (person.surname || '') + '|' + (person.name || '') + '|' + (person.patronymic || '');
      // v9.3: «Фамилия И.И.» и «И.И. Фамилия» — разные ключи: женская
      // именительная форма омонимична мужскому родительному, без разделения
      // жена («И.И. Иванова») сливалась с мужем («Иванов И.И.») или матерью
      if (person.surname && !person.name && person.initials.length > 0) {
        // поверхностная форма важна: лемма «иванов» стирает разницу между
        // мужем «И.И. Иванов» и женой «И.И. Иванова»
        var surfKey = (person.surnameSurface || person.surname).toLowerCase().replace(/[^а-яёіўa-z-]/g, '');
        key = person.surname + '||' + surfKey + '|' + person.initials.join('.') + '|' + (person.surnameAfterInitials ? 'pre' : 'post');
      }
      // «Кузнецов Д.С.» сливается с уже найденным «Кузнецов Дмитрий Сергеевич»
      // (иначе одна персона получает два номера псевдонима)
      // Персона без фамилии, но с полной парой имя+отчество (артефакт парсинга
      // инициалов «Тарасевич И.В.») — сливаем по имени+отчеству
      if (!person.surname && person.name && person.patronymic) {
        var npc = byNamePatr[person.name + '|' + person.patronymic] || [];
        if (npc.length > 0) { personByKey[key] = npc[0]; }
      }
      if (person.surname && !person.name && person.initials.length > 0) {
        // нормализуем обе стороны: лемма «бондаря» маршрутами морфологии
        // может отличаться от «бондарь» в зависимости от контекста
        var pSur = normSurnameLemma(person.surname) || person.surname;
        var cands = bySurname[pSur] || [];
        for (var ci = 0; ci < cands.length; ci++) {
          var c = cands[ci];
          var cSur = normSurnameLemma(c.surname) || c.surname;
          if (cSur !== pSur) continue;
          // v9.3: женская поверхностная форма фамилии не склеивается с мужчиной,
          // если инициалы стоят ПЕРЕД фамилией (женская нотариальная форма
          // «И.И. Иванова»). «Иванова» и жен. (именит.) и муж. (род.п.) —
          // омонимы; морфология всегда отдаёт lemma=иванов/gender=m, поэтому
          // «И.И. Иванова» (жена) ошибочно сливалась с мужем «Иванов И.И.»
          // по совпавшим инициалам. При обратном порядке «Иванова П.И.»
          // (фамилия+инициалы) это обычно род.падеж мужчины/ребёнка —
          // склейку сохраняем.
          var surf = person.surnameSurface || '';
          if (person.surnameAfterInitials && c.gender === 'm' && /(ова|ева|ина|ына|ская|цкая|ая)$/i.test(surf)) continue;
      ...
      // v9.3: фамилия-одиночка («(Петрова)» из подписи) не наследует персону
      // с полными данными. Иначе personByKey[«фамилия||»], прописанный при
      // инициал-склейке («А.В. Петрова» → Петрова Анна В.), перекрепляет
      // девичью фамилию жены к однофамильнице-представителю. Голые фамилии
      // склеиваются только между собой (общий placeholder на все упоминания).
      if (personByKey[key] && person.surname && !person.name && !person.patronymic && person.initials.length === 0) {
        var tgt = personByKey[key];
        if (tgt.name || tgt.patronymic || tgt.initials.length > 0) personByKey[key] = null;
      }
      if (!personByKey[key]) {
        ...
        if (person.name && person.patronymic) { var npK = person.name + '|' + person.patronymic; (byNamePatr[npK] = byNamePatr[npK] || []).push(person); }
      }
