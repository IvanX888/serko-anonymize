...
(function (root, factory) {
  ...
  function applyReplacements(text, matches, mode, persons) {
    ...
    var polisPlaceholders = {};    // нормализованный номер -> placeholder
    var pseudonymCache = {};
    ...
    function placeholderFor(m) {
      ...
      // Консистентность: один номер полиса — один номер маски (текст = идентификатор)
      if (m.type === 'polis') {
        var polKey = m.text.replace(/\s+/g, '');
        if (polisPlaceholders[polKey]) return polisPlaceholders[polKey];
      }
      ...
      if (m.type === 'polis') polisPlaceholders[m.text.replace(/\s+/g, '')] = ph;
      return ph;
    }
