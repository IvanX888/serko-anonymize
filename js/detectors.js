...
(function (root, factory) {
  ...
  function detectPassportRU(text, protectedSpans) {
    ...
    var re3 = /(?:водительск(?:ое|ие|их|ого|им)?\s*(?:удостоверени[еяю]|прав[а-я]*)|СТС|ПТС|свидетельство о регистрации(?:\s+ТС)?)[:\s]*(?:№[\s]*)?([0-9]{2}[\s-]?[0-9А-ЯA-Z]{2}[\s-]?[0-9]{6})/gi;
    ...
    var re4 = /серия\s+([0-9]{4}[\s]*№[\s]*[0-9]{6})(?![0-9])/gi;
  ...
  function detectAddresses(text, protectedSpans) {
    ...
    var bareHouse = '([0-9]{1,4}[А-Яа-яA-Za-z]?(?:[-/][0-9]{1,3}[А-Яа-я]?)?(?:[,\\s]+[0-9]{1,4}[А-Яа-я]?)?)(?![.][0-9])(?![/][0-9])(?![.,]\\s)(?![\\s.,;]*(?:г|года|год|лет|проц|%)[\\s.,;])(?!\\s*(?:' + monthsAlt + '))';
    ...
    var re2 = new RegExp(wordBoundary + '(?:' + regionAny + '[,\\s]+)?' + '(?:' + cityBlock + '|[А-ЯЁІЎ][а-яёіў]+(?:[\\s-][А-ЯЁІЎа-яёіў]+)?)' + '[,\\s-]+' + streetAdj + streetBlock + '[,\\s-]+' + '(?:' + houseBlock + '|' + bareHouse + ')', 'gi');
    while ((m = re2.exec(text)) !== null) {
      if (inSpans(protectedSpans, m.index, m.index + m[0].length)) continue;
